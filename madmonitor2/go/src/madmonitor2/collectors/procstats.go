...
func procstats() {
	...
	for {
		...
		for scanner.Scan() {
			...
			inc.MsgQueue <- fmt.Sprintf("procstats system.cpu.load[percpu,avg1] %v %v\n", ts, m[0][1])
			inc.MsgQueue <- fmt.Sprintf("procstats system.cpu.load[percpu,avg5] %v %v\n", ts, m[0][2])
			inc.MsgQueue <- fmt.Sprintf("procstats system.cpu.load[percpu,avg15] %v %v\n", ts, m[0][3])
