...
class clsGeneric {
    ...
    function __construct($client_message) {
        ...
        $this->m_summary = array("load", "uptime_day", "uptime_his", "tcp_connections"); 
        $this->m_cpu = array("use", "nice", "system", "interrupt", "idle");
        $this->m_mem = array("active", "inact", "wired", "cache", "buf", "free");
        $this->m_swap = array("total", "used", "free", "inse");
        $this->m_disk = array(array("mounted" => "", "capacity" => "", "iused" => ""));
        $this->m_process = array("sum", "starting", "running", "sleeping", "stopped", "zombie", "waiting", "lock");
        $this->m_network = array(array("ifname" => "", "in" => "", "out" => ""));
        $this->m_link = array(array("sserver" => "", "dserver" => "", "flow" => ""));
        $this->m_service = array(array("name" => "", "port" => "", "status" => ""));
    }
    ...
    function clsGeneric($client_message) {
        ...
        global $module_name,$debug_level;
        $this->m_module_name = $module_name;
        $this->m_debug_level = $debug_level;
    }
    ...
    function getServerName() {
        ...
        $this->m_server = $Ret;
        DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getServerName return:$Ret]");
        return ($Ret);
    }
    ...
    function getSummary() {
        ...
        DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getSummary return:$Ret]");
    ...
    function getCpu() {
        ...
        if (empty($cpu_str)) { // 允许客户端不传递此项 
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getCpu return:$Ret]");
            return ($Ret);
        }
    }
    ...
    function getMem() {
        ...
        if (empty($mem_str)) { // 允许客户端不传此项 
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getMem return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getSwap() {
        ...
        if (empty($swap_str)) { // 允许客户端不传此项 
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getSwap return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getDisk() {
        ...
        if (empty($disk_str)) { // 允许客户端不传此项 
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getDisk return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getProcess() {
        ...
        if (empty($process_str)) {
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getProcess return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getNetwork() {
        ...
        if (empty($network_str)) {
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getNetwork return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getLink() {
        ...
        if (empty($link_str)) {
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getLink return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getService() {
        ...
        if (empty($service_str)) {
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][service_str not empty]");
            // 劈开service为各连接明细数组
            $service_arr = is_array(explode(__SOURCE_SPLIT_TAG3, $service_str)) ?explode(__SOURCE_SPLIT_TAG3, $service_str) :false; 
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][service_arr:".join(',',$service_arr)."]");
            // 对每个service回调检查其类型
            $check_ok = (false!=$service_arr) ?(in_array(false, array_map("validService", $service_arr)) ?false :true) :false;
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getService][check_ok:$check_ok]");
            if (false!=$check_ok) {
            ...
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getService return $Ret]");
            return ($Ret);
        }
    }
    ...
    function getAllInfo() {
        ...
        if (!$Ret) {
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getAllInfo fail!]");
        } else {
            DebugInfo(3,$this->m_debug_level,"[$this->m_module_name][$this->m_class_name][getAllInfo ok!]");
        }
        return ($Ret);
    }
}
