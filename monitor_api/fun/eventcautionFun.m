...
/*
  ...
  | Create:
  +----------------------------------------------------------------------+
  | Last-Modified: 2012-12-20 14:16:57
  +----------------------------------------------------------------------+
 */
            ...
            try {
                if (!empty($host)) {
                    ...
                    foreach ($arr as $eventCode => $eventVal) {
                        ...
                        if ($eventCode!=__EVENTCODE_DOWN && strtolower($event_level)=='c') { // 注意事件 
                            list($event_status, $event_desc) = explode('|', $eventVal->value); // 取得事件描述
                            /* 构造输出数组 */
                            in_array($event_id, (array)$needfixArr) && $event_status==__EVENT_ACTIVE && @$temp_page[] = array($host, $event_item_map_table[$event_id][__EVENT_LANG_CHS], $event_id, __EVENT_CLASS_CAUTION, $duration_time, date("Y-m-d H:i:s", $upload_time[$host]), $uiWordArr[$host][$event_id]);
                        }
                    }
                }
            } catch (Exception $e) { }
