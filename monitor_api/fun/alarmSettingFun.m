...
case(__OPERATION_READ): //查询操作 
    if($GLOBALS['selector'] == __SELECTOR_SINGLE && $_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if (empty($_CONFIG['alarm_interval']['current_engine'])) {
            $_CONFIG['alarm_interval']['current_engine']="N/A";
            $_CONFIG['alarm_interval']['all_default_gp_down']=3600;
            $_CONFIG['alarm_interval']['all_cust_gp_down']=3600;
            $_CONFIG['alarm_interval']['one_default_gp_down']=3600;
            $_CONFIG['alarm_interval']['one_cust_gp_down']=3600;
            $_CONFIG['alarm_interval']['one_default_server_down']=3600;
            $_CONFIG['alarm_interval']['one_cust_server_down']=3600;
            $_CONFIG['alarm_interval']['general_server_event']=3600;
            $_CONFIG['alarm_interval']['recover_notifiction']=3600;
        }
        $return_str = <<<EOT
{
