...
function get_userGroup_data_Callback(json, textStatus, jqXHR){
	if(json==null){
	  console.log("json null");
		...
		//console.log(json);
		var usergroup_name=url_params("name"),row1=1,edit_user_name=["用户名","描述"],selected=[],per_name=[],permission=[];
...
/**************************add_usersgroup memeber 添加到表格里**********************************************************/	
  $("#add_user_submit").click(function(){
	  ...
	  if(obj.length==0){
		  var add_user_content=["用户名","描述"],row=1;
    ...
    $("#userGroup_member_table tr td").each(function(m){
     ...
     window.location.href="usergroupManager.html?version="+version;
